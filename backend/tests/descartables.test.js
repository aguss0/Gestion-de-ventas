...
test('migración aditiva, importación y ventas en una base temporal', { timeout: 240000 }, async () => {
  ...
  try {
    ...
    app.use('/historial-listas', require('../src/routes/historialListas'));
    ...
    const historialBase = base.replace('/descartables', '/historial-listas');
    const historico = await fetch(historialBase, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ tipo: 'descartables', recargoUnidad: 30, recargoBulto: 20, items: [{ codigo: a.codigo, nombre: a.nombre, unidadesBulto: 12, precioUnidad: 2097.99, precioBulto: 23239.26 }] }) });
    assert.equal(historico.status, 201);
    const historicoCreado = await historico.json();
    assert.equal(historicoCreado.cantidad, 1);
    assert.equal(historicoCreado.items[0].precioBulto, 23239.26);
    const historialGuardado = await (await fetch(historialBase)).json();
    assert.equal(historialGuardado[0].cantidad, 1);
    assert.equal(historialGuardado[0].items, undefined);
    assert.equal((await (await fetch(historialBase + '/' + historicoCreado.id)).json()).items[0].precioUnidad, 2097.99);
